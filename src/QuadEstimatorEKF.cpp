...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(0) = predictedState(0) + dt * curState(3);
  predictedState(1) = predictedState(1) + dt * curState(4);
  predictedState(2) = predictedState(2) + dt * curState(5);

  V3F att_if = attitude.Rotate_BtoI(accel);

  predictedState(3) = predictedState(3) + dt * att_if.x;
  predictedState(4) = predictedState(4) + dt * att_if.y;
  predictedState(5) = predictedState(5) + dt * att_if.z - dt * static_cast<float>(CONST_GRAVITY);
